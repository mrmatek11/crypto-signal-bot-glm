    ...
    def _build_signal_embed(self, signal: Signal) -> dict:
        ...
        if source:
            source_names = {
                ...
                "STOCH-ONLY": "🔵 Stoch Zone Signal",
                "STOCH STRICT+NWO": "🎯 Strict Stoch + 🧠 NWO",
            }
            fields.append({
                "name": "Źródło",
        ...
        # Confidence level
        confidence = signal.extra_data.get("confidence", "MEDIUM")
        conf_emoji = {"HIGH": "🟢", "MEDIUM": "🟡", "LOW": "🔴"}.get(confidence, "⚪")
        fields.append({
            "name": "Pewność",
            "value": f"{conf_emoji} {confidence}",
            "inline": True
        })
        ...
        # ─── GLM AI Analysis ────────────────────────────────────────────
        ai_score = signal.extra_data.get("ai_score")
        ai_recommendation = signal.extra_data.get("ai_recommendation")
        ai_analysis = signal.extra_data.get("ai_analysis")
        if ai_score is not None:
            score_emoji = {range(8, 11): "🟢🔥", range(6, 8): "🟢", range(4, 6): "🟡"}.get(
                next((k for k in [range(8, 11), range(6, 8), range(4, 6)] if ai_score in k), None), "🔴"
            )
            rec_emoji = {"TAKE": "✅", "WATCH": "👀", "SKIP": "❌"}.get(ai_recommendation, "?")
            ai_text = f"{score_emoji} **{ai_score}/10** {rec_emoji} {ai_recommendation or ''}"
            if ai_analysis:
                ai_text += f"\n💡 {ai_analysis[:150]}"
            
            ai_risks = signal.extra_data.get("ai_risks", [])
            if ai_risks:
                ai_text += f"\n⚠️ Risks: {', '.join(ai_risks[:2])}"
